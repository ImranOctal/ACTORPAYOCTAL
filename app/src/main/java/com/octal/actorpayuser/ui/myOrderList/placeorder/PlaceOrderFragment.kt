    ...
    fun setAdapter() {
        val adapter = CheckoutShippingListAdapter(
            ...
            if (action == Clicks.Edit) {
                ...
                CommonDialogsUtils.showCommonDialog(requireActivity(),placeOrderViewModel.methodRepo,
                    ...
                    is ResponseSealed.Success -> {
                        ...
                        when (event.response) {
                            ...
                            is PlaceOrderResponse ->{
                                PlaceOrderDialog(requireActivity(),placeOrderViewModel.methodRepo,event.response.data){
                                    cartViewModel.cartItems.value.clear()
                                    if(it == "order"){
                                        val navOptions= NavOptions.Builder().setPopUpTo(R.id.homeBottomFragment,false).build()
                                        Navigation.findNavController(requireView()).navigate(R.id.myOrderFragment,null,navOptions)
                                    }
                                    if(it == "shopping"){
                                        val navOptions= NavOptions.Builder().setPopUpTo(R.id.homeBottomFragment,false).build()
                                        Navigation.findNavController(requireView()).navigate(R.id.productListFragment,null,navOptions)
                                    }
//                                    startActivity(
